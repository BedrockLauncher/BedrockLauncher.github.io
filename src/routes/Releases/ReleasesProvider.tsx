...
export const ReleasesContext = createContext({ public: [], beta: [] } as {
  public: GitHubReleases[];
  beta: GitHubReleases[];
});
const ReleasesProvider: FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const getReleases = async (going: boolean) => {
    ...
    if (!going) return;
    setPublics(
      ((await res1.json()) as GitHubReleases[]).filter(
        (p) => p.name !== '0.0.0.0'
      )
    );
