...
const FAQDetails: FC<{ style?: React.CSSProperties; title: string; desc: string; githubURL?: string; usesMD?: boolean; replaceURLs?: boolean }> = ({
  title,
  desc,
  githubURL,
  usesMD,
  ...
  return (
    <div className={'faqdetails' + (openSemi ? ' open-semi' : '') + (open ? ' open' : '')} style={style}>
      <div className="faqdetails-header" tabIndex={0} onClick={handleOpen} onKeyDown={handleKeyDown}>
        <span>
          <DownArrow />
        </span>
        {title}
